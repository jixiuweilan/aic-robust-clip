...
执行 agent 在后续执行时自行向对应组员确认连接地址、工作目录、获准使用的 GPU；本次交付不建立 SSH 连接。组员直接从 GitHub 获取任务说明和代码，按任务通知核对两台的完整 Git 提交号及源码摘要；不能只复制某个修复文件后沿用旧准入。无需用户另发材料包。
...
2026-09-20 修复：`select`、`accept`、`run` 现在共用两机选择校验，检查 A/B 对应关系、不同主机指纹、相同源码/依赖/Python/CUDA/资产/配方、六组结果及合法耗时，并重新计算共同通过项、排序与 5% 内的选择规则。复制单机摘要、只改 member、缺少另一台通过项或擅改选中 profile，都会在启动训练前被拒绝。

新 `selection.json` 和 `acceptance.json` 使用 `schema_version=2`。选择记录保存嵌入摘要的规范化内容摘要；准入回执显式保存 `selection_path`、`selection_sha256`。`run` 必须读取这个被绑定的选择文件再次复核，不仅检查回执状态或文件哈希。旧/手工回执不自动升级，不得给旧证据补改版本号或哈希来放行。两台恢复工作后应在统一的新源码上重新准备及验收；当前停工状态不变。此校验验证证据的一致性，不等同于对远端主机的签名认证，负责人仍须核对两台各自原始证据。

本次修复验证：4060 专项 13 项、全套 126 项通过，零失败零跳过；compileall、pip check、git diff --check 通过。回归包含收到的单机 A/B 复制结构、改名后同主机、peer 身份/配方不匹配、失败 profile、非法耗时、排序/摘要改动、重写选择文件哈希后尝试运行、缺少显式选择绑定，以及正常 B 机不访问 A 机文件路径的完整流程。
