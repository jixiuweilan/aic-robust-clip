...
2026-09-22：用户明确只推进复赛，初赛 CE → candidates、TURN 重试及初赛机器
准入安排全部撤回。旧说明和交付包仅作历史记录，不再执行。UUID 工程修复保留，
不能以验证该修复为由继续初赛训练。当前未操作远端进程，远端状态由操作 agent 核实。

复赛审计观察器已记录 `audit_failed_or_interrupted`，尚无完整可用的资产描述，
此前“正在运行”的状态已过期；该记录不能证明数据损坏。下一步先在稳定执行环境
完成复赛训练包的解码、重复组审计及 seed17 划分，随后在训练机器生成唯一公共
HEAD20-GCE，完成新准入，再释放四张 T4 的 full_visual CE/TURN/FINE/SNSCL。
两张 4060 保留各自 LoRA CE→TURN、CE→FINE 规划，当前不安排组员执行。
全部维持 30 轮计划、第 10 轮验证和保存后暂停，不读取 test。

2026-09-20：本轮交付代码、配置工具和合成正确性测试。用户最新要求为组员休息，
**不准备或发布组员任务，不安排其验收或训练**。本页是实现说明，不是执行任务书。
T4 执行指令没有上传 GitHub，也未远程执行任何操作。

2026-09-21更新：用户已取得官方复赛包，位于本机`data1/`。只读取train.zip，
目录清单为148,695张图、750类，每类5–248张，归档SHA256为
`11b70e8b86e7b093cfeb117f5a8422b38c0f7011e41344cc14ae80bc29f3d319`。
`test.zip`未打开。训练包以只读使用的硬链接登记到
`data/second_round/received-20260921/train.zip`，未修改或重打包原文件。
审计输出独立保存在`outputs/second_round/received-20260921-audit-v1/`；
实际完成状态以该目录的审计证据为准，目录清单不能代替完整解码与重复组审计。
已补`--member-prefix train`以支持官方ZIP的`train/类别/图片`布局，原成员名和字节保持不变，
默认旧布局行为保持不变。`data1/`已加入忽略规则，避免提交竞赛数据。

当前尚未完成 HEAD20-GCE 初始化、CUDA 测速或新机器准入，八个复赛 run 尚不能开跑。
配置中的`blocked_on_round2_assets`表示仍需完整可用的资产描述及新初始化，
不能据此把“已收到原始ZIP”误写成“数据尚未取得”。本机没有下载、真实数据特征提取或
正式训练。旧初赛目录、HEAD3、检查点、旧配方摘要与恢复入口保持原样。

## 已撤回的初赛独立验证（历史记录）

以下记录保留实现与历史授权背景，2026-09-22 起不再构成执行授权。
多卡增量入口、同期CE/FINE/SNSCL及4060仅准入的历史安排见
[初赛同期对照与机器准备](preliminary-comparisons.md)。旧TURN入口保留供回归。
...
正常收敛但某类零可信样本的情况按下述显式弃权规则处理，不能记成可信筛选成功。
...
## 初赛 TURN 显式弃权变体（2026-09-21）

`064aca0` 的服务器回传表明：class 325 已在第378次满足1e-6收敛容差，
但后验阈值0.6没有选出样本。使用回传的两分量参数解析求取后验上界，
该评分区间的最大低损失分量后验约为0.554800；这解释了该次零选中，
不能推断该类别没有噪声，也不能据此断言其他类别必然失败。
原始逐样本分数未回传，合成测试只重用聚合拟合参数验证数学性质，
不冒称重现了真实数组的EM过程。

工程决策：初赛独立入口采用 `TURN-ABSTAIN-B32-v1`，配置版本为
`preliminary-turn-abstain-pilot-v2`，显式声明
`zero_selection_policy=retain_observed`。仅在GMM正常收敛且某类无人达到0.6时，
该类本次弃权筛选，全部保留原标签CE监督；不改标签、不降阈值、不补top-k，
不把保留样本标为可信。下一次完整train评分重新判断，不永久豁免该类。
这是新的项目配方，不能把旧任务的失败改记为通过。

少于8样本/恒分仍走原有不筛选分支。非有限输入、分量退化、未收敛仍失败；
固定1000次求解上限、容差/方差下限1e-6不变。复赛入口、FINE/SNSCL和默认
`select`仍采用`error`，拒绝零选中；不向复赛自动推广该变体。
初赛seed17、LoRA、HEAD3、microbatch32、workers2、有效batch128、FP16、
FP32评分/dev64、30轮调度及第10轮评分/验证/保存后暂停均保持不变。

profile与正式训练共用同一策略。profile新增`selection.json`聚合诊断，
记录实际后验范围、解析区间上界与阈值可达性；逐类及汇总区分
`confident_selected`与`retained_observed`，另报弃权类数/样本数/比例、
未拟合类数及非有限输入数。`nonfinite_count`计数原始评分输入，
不把未拟合概率的NaN哨兵当作评分损坏。诊断不含样本ID或逐样本评分。
逐轮及最终摘要保留这些统计，评估模型收益时必须同时检查弃权覆盖。
TURN的`reliability_weight_mean`不是可信比例；其CE权重为1，可信计数以上述字段为准。

弃权检查点采用`turn-abstain-method-v1`并绑定策略身份，拒绝与旧严格策略互相恢复。
旧失败目录原样保留；新源码必须重新通过全套测试、FP32/FP16合成CUDA检查、
固定profile和三次dev窗口后，才从原HEAD3进入新run。首次交付仅为本机软件验证，
真实数据的弃权覆盖、速度和分数仍待操作agent回传。

本机验收：新增5项弃权专项通过；最终全套171项通过（29.361秒），
零失败、零错误、零跳过。覆盖解析上界、双峰/少样本混合统计、严格策略仍失败、
旧策略回执拒绝、配置隔离、弃权状态/RNG恢复后一致更新、逐轮摘要及十轮暂停/dev回放。
既有重复值、近常数、短窗口和GMM确定性回归保留。compileall、pip check、
git diff --check通过。日志为`/tmp/aic-abstain-focused.log`与
`/tmp/aic-abstain-final-suite.log`；均为合成正确性检查，不是新版本服务器准入或分数证据。
