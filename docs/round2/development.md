...
2026-09-23：原六卡资源不可用，首批改为单张 4090 串行执行。当前任务见
[单卡 4090 说明](cloud4090-execution.md)；首批最多 LoRA CE、全视觉 CE 和
同适配方式的 TURN 三支，FINE/SNSCL 暂不启动。下方六卡安排仅为历史记录，
不是当前开跑指令。复赛审计、HEAD20 和新机准入仍待真实执行回执。

## 2026-09-22 历史执行链

当前执行说明见 [组员操作说明](team-execution.md)。下方旧验收数字和“组员休息”描述
均按各段日期保留为历史记录；不代表本轮状态。原始 train.zip 已登记，旧审计中断，
需要服务器重新完整审计。真实公共 HEAD20、六卡准入和八个正式 run 尚无本轮回传。

新增 `round2.delivery` 专用任务：持久日志/退出状态、独立进程会话、公共资产串联、
真实初始评分及方法可行性检查、按组准入/配置、轮末观察后恢复及学生 dev 回放。
审计输出从开始即存在，失败或中断不能加载成训练资产；新审计要求保留成功状态文件。
HEAD20/训练配方不变；GMM1000、严格零选中停止、原阈值/容差不变。新源码须重做准入。
真实可行性检查对全 train 评分后做有效 batch128 的两次更新，SNSCL 使用第六轮活动路径；
不保存权重。准入检查这些证据先于测速窗口，方法失败停止所属组。
正式 `run --stop-after-epoch {1,5,6,10}` 只增加轮末观察点，不缩短30轮调度。
交付入口首轮核对指标、best/last、完整状态和摘要，SNSCL另外核对第五/六轮，
然后恢复同一 run 至第十轮；到第十轮仍拒绝继续。

本轮完整本机验收结果见 [验收记录](validation-20260922.md)，仅为合成软件正确性证据。
