...
class PerformanceConfig:
    ...
    def from_config(cls, config):
        ...
        result = cls(**{**dict.fromkeys(("cache_batch_size", "eval_batch_size", "head_batch_size"), micro), **value})
        ...
        if type(result.num_workers) is not int or not 0 <= result.num_workers <= 16:
            raise ValueError("performance.num_workers must be an integer between 0 and 16")
        ...
        if config.get("execution_mode", "smoke") == "smoke" and (
                ...
                or result.num_workers or result.pin_memory):
